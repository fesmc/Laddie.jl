...
Base.show(io::IO, v::Var{LX, LY, FT}) where {LX, LY, FT} = print(io,
    "Var{", nameof(LX), ", ", nameof(LY), "}(", FT, ", ",
    size(v.present, 1), "×", size(v.present, 2), ", levels: past/present/future)")
...
function _show_forcing(io::IO, f::AbstractForcing)
    ...
    print(io, nameof(typeof(f)), "{", eltype(f.Tz), "}: ", length(f.z),
          "-point profile, z ∈ [", zlo, ", ", zhi, "] m, T ∈ [",
          round(Tlo; digits = 3), ", ", round(Thi; digits = 3), "] °C, S ∈ [",
          round(Slo; digits = 3), ", ", round(Shi; digits = 3), "] psu")
end
...
function Base.show(io::IO, g::Grid{FT}) where {FT}
    print(io, "Grid{", FT, "}: ", g.Ny, "×", g.Nx, " cells (", g.Ny - 2, "×",
          g.Nx - 2, " interior), dx = ", g.dx, " m, dy = ", g.dy, " m")
end

function Base.show(io::IO, ::MIME"text/plain", g::Grid)
    show(io, g)
    msk = g.mask
    print(io, "\n  cells: ", count(==(3), msk), " shelf, ", count(==(2), msk),
          " grounded, ", count(==(0), msk), " ocean, ", count(==(1), msk),
          " land/border")
end
...
function Base.show(io::IO, ::MIME"text/plain", p::Params{FT}) where {FT}
    ...
    scal = [(fn, getfield(p, fn)) for fn in fieldnames(typeof(p))
            if getfield(p, fn) isa Number]
...
function Base.show(io::IO, m::Model{FT}) where {FT}
    g = getfield(m, :grid)
    print(io, "Model{", FT, "} on ", _backend_name(m), ": ", g.Ny - 2, "×",
          g.Nx - 2, " interior, ", nameof(typeof(getfield(m, :forcing))), " forcing")
end
