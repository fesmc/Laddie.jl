...
struct Grid{FT, A<:AbstractMatrix{FT}}
    ...
    tmaskym1::A;  tmaskyp1::A
    tmaskxm1::A;  tmaskxp1::A
    tmaskxm1ym1::A; tmaskxm1yp1::A
    tmaskxp1ym1::A
    ...
    umask::A; vmask::A
    umaskym1::A; umaskyp1::A
    umaskxm1::A; umaskxp1::A
    vmaskym1::A; vmaskyp1::A
    vmaskxm1::A; vmaskxp1::A

    tmask_im::A; tmask_ip::A
    tmask_jm::A; tmask_jp::A
    umask_im::A; umask_ip::A
    umask_jm::A; umask_jp::A
    vmask_im::A; vmask_ip::A
    vmask_jm::A; vmask_jp::A
end
...
function Grid(mask::AbstractMatrix{Int}, zb::AbstractMatrix, dx, dy; FT = Float64)
    ...
    ocnym1 = ym1(ocn);     ocnyp1 = yp1(ocn)
    ocnxm1 = xm1(ocn);     ocnxp1 = xp1(ocn)

    tmaskym1 = ym1(tmask); tmaskyp1 = yp1(tmask)
    tmaskxm1 = xm1(tmask); tmaskxp1 = xp1(tmask)
    ...
    grlE = grd .* tmaskxp1;   grlW = grd .* tmaskxm1
    grlN = grd .* tmaskyp1;   grlS = grd .* tmaskym1
    grl = grlE .+ grlN .+ grlW .+ grlS

    # Velocity masks
    umask = (tmask .+ isfW) .* (o .- xm1(grlE))
    vmask = (tmask .+ isfS) .* (o .- ym1(grlN))
    umaskym1 = ym1(umask); umaskyp1 = yp1(umask)
    umaskxm1 = xm1(umask); umaskxp1 = xp1(umask)
    vmaskym1 = ym1(vmask); vmaskyp1 = yp1(vmask)
    vmaskxm1 = xm1(vmask); vmaskxp1 = xp1(vmask)

    # Stagger-count denominators
    tmask_im = tmask .+ tmaskxp1; tmask_ip = tmask .+ tmaskxm1
    tmask_jm = tmask .+ tmaskyp1; tmask_jp = tmask .+ tmaskym1
    umask_im = umask .+ umaskxp1; umask_ip = umask .+ umaskxm1
    umask_jm = umask .+ umaskyp1; umask_jp = umask .+ umaskym1
    vmask_im = vmask .+ vmaskxp1; vmask_ip = vmask .+ vmaskxm1
    vmask_jm = vmask .+ vmaskyp1; vmask_jp = vmask .+ vmaskym1
    ...
    Grid{FT, Matrix{FT}}(
        ...
        umask, vmask,
        umaskym1, umaskyp1, umaskxm1, umaskxp1,
        vmaskym1, vmaskyp1, vmaskxm1, vmaskxp1,
        tmask_im, tmask_ip, tmask_jm, tmask_jp,
        umask_im, umask_ip, umask_jm, umask_jp,
        vmask_im, vmask_ip, vmask_jm, vmask_jp,
    )
end
