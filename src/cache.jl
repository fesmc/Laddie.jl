...
mutable struct Cache{FT, A<:AbstractMatrix{FT}, GamT, Conv2}
    ...
    melt::A; Tb::A; Tf::A
    Ta::A;   Sa::A; drho::A
    ustar::A; entr::A; detr::A
    ent2::A;  nentr::A; ent::A
    Sb::A;   drhob::A; convection::A
    convD::A; dDdt::A; Ddrho::A
    conv2::Conv2
    gamT::GamT; gamS::GamT
    # Advection stencil temporaries
    Dym1::A; Dyp1::A
    Dxm1::A; Dxp1::A
    Dxm1ym1::A; Dxp1ym1::A; Dxm1yp1::A
    Vyp1::A; Uxp1::A
    Upos::A; Uneg::A; Vpos::A; Vneg::A
    Vyp1pos::A; Vyp1neg::A; Uxp1pos::A; Uxp1neg::A
    Vip::A; Vim::A; Uip::A; Uim::A
    Vjp::A; Vjm::A; Ujp::A; Ujm::A
    signU::A; signV::A
...
function Cache(FT::Type, MP::Type, CS::Type, ny::Int, nx::Int)
    ...
    Cache{FT, Matrix{FT}, GamT, Conv2}(
        copy(z), copy(z), copy(z),          # melt, Tb, Tf
        copy(z), copy(z), copy(z),          # Ta, Sa, drho
        copy(z), copy(z), copy(z),          # ustar, entr, detr
        copy(z), copy(z), copy(z),          # ent2, nentr, ent
        copy(z), copy(z), copy(z),          # Sb, drhob, convection
        copy(z), copy(z), copy(z),          # convD, dDdt, Ddrho
        conv2_init, gamT_init, gamS_init,   # conv2, gamT, gamS
        copy(z), copy(z),                   # Dym1, Dyp1
        copy(z), copy(z),                   # Dxm1, Dxp1
        copy(z), copy(z), copy(z),          # Dxm1ym1, Dxp1ym1, Dxm1yp1
        copy(z), copy(z),                   # Vyp1, Uxp1
        copy(z), copy(z), copy(z), copy(z), # Upos, Uneg, Vpos, Vneg
        copy(z), copy(z), copy(z), copy(z), # Vyp1pos, Vyp1neg, Uxp1pos, Uxp1neg
        copy(z), copy(z), copy(z), copy(z), # Vip, Vim, Uip, Uim
        copy(z), copy(z), copy(z), copy(z), # Vjp, Vjm, Ujp, Ujm
        copy(z), copy(z),                   # signU, signV
        copy(z), copy(z), copy(z), copy(z), # D0ip, D0im, D0jp, D0jm
